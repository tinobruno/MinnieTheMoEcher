...
public:
    ...
    std::string generate(const std::vector<int>& prompt, int max_tokens = 512,
        ...
        // Prefill prompt (batched for all-resident DeepSeek V4, sequential otherwise)
        auto prefill_start = std::chrono::high_resolution_clock::now();
        if (enable_batched_prefill_ && cfg_.architecture == ModelArch::DEEPSEEK_V4 && prompt.size() > 1) {
            ...
            double elapsed_sec = std::chrono::duration<double>(prefill_end - prefill_start).count();
            if (prompt.size() > 0 && elapsed_sec > 0.0) {
                LOG_INFO("[PREFILL STATS] %zu prompt tokens prefilled in %.3fs -> %.2f tok/s (sequential)",
                         prompt.size(), elapsed_sec, (double)prompt.size() / elapsed_sec);
            }
        }
