...
s1_template = """    if (request.contains("tools") && request["tools"].is_array() && !request["tools"].empty()) {
...
s2_template = """    if (request.contains("tools") && request["tools"].is_array() && !request["tools"].empty()) {
...
content = content.replace(s1_template, s2_template)

# 2. Update tool_choice requirement injection
s1_req = """            if (request.contains("tool_choice") && request["tool_choice"].is_string() && request["tool_choice"].get<std::string>() == "required") {
                result += "<\\xef\\xbd\\x9c" "tool\\xe2\\x96\\x81" "calls\\xe2\\x96\\x81" "begin\\xef\\xbd\\x9c>";
            }"""
s2_req = """            if (request.contains("tool_choice") && request["tool_choice"].is_string() && request["tool_choice"].get<std::string>() == "required") {
                result += "<\\xef\\xbd\\x9c" "DSML\\xef\\xbd\\x9ctool_calls>\\n";
            }"""
content = content.replace(s1_req, s2_req)

# 3. Update the tags
content = content.replace('tool_detect_buffer.find("<\\xef\\xbd\\x9c" "tool\\xe2\\x96\\x81calls\\xe2\\x96\\x81" "begin" "\\xef\\xbd\\x9c>")', 'tool_detect_buffer.find("<\\xef\\xbd\\x9c" "DSML\\xef\\xbd\\x9ctool_calls>")')
content = content.replace('tool_detect_buffer.find("<\\xef\\xbd\\x9c" "tool\\xe2\\x96\\x81calls\\xe2\\x96\\x81" "end" "\\xef\\xbd\\x9c>")', 'tool_detect_buffer.find("</\\xef\\xbd\\x9c" "DSML\\xef\\xbd\\x9ctool_calls>")')

# 4. Update the token type classification
s1_class = """                    if (in_think_block) token_type = 1;
...
s2_class = """                    if (in_think_block) token_type = 1;
                    else if (in_tool_block || tool_detect_buffer.find("<tool_call>") != std::string::npos ||
                             tool_detect_buffer.find("<\\xef\\xbd\\x9c" "DSML\\xef\\xbd\\x9ctool_calls>") != std::string::npos) token_type = 2;"""
content = content.replace(s1_class, s2_class)

# 5. Replace parser logic
# Use indexing to replace exactly from `std::string begin_tag` to `if (tool_calls_json.empty()) {`
idx1 = content.find('                            std::string begin_tag = "<\\xef\\xbd\\x9c" "tool\\xe2\\x96\\x81call\\xe2\\x96\\x81" "begin" "\\xef\\xbd\\x9c>";')
idx2 = content.find('                            if (tool_calls_json.empty()) {\n                                // Fallback: try parsing as a JSON array (old format)')

if idx1 != -1 and idx2 != -1:
    s_parse = """                            std::string invoke_start = "<\\xef\\xbd\\x9c" "DSML\\xef\\xbd\\x9cinvoke name=\\\"";
...
\n"""
    content = content[:idx1] + s_parse + content[idx2:]
